...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		if (((TreeIterator) findMax(subtree.root)).currentNode.data > ((TreeIterator) findMin(this.root)).currentNode.data) {
			return;
		}
		BSTNode subParent = ((TreeIterator) findMin(this.root)).currentNode;
		BSTNode subRoot = subtree.root;
		subParent.left = subRoot;
		subRoot.parent = subParent;	
		this.size += subtree.size;
